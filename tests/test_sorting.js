...
// If indexedDB not already mocked in global scope, set up lightweight mock
            ...
            const tx = {
              ...
              objectStore: () => ({
                ...
                getAll: () => {
                  const r = {};
                  setTimeout(() => {
                    r.result = Array.from(inMemoryData.values());
                    if (r.onsuccess) r.onsuccess({ target: r });
                  }, 0);
                  return r;
                },
                index: (indexName) => ({
                  openCursor: (range, direction = 'next') => {
                    const r = {};
                    setTimeout(() => {
                      const records = Array.from(inMemoryData.values());
                      if (direction === 'prev') {
                        records.sort((a, b) => (b.capturedAt || 0) - (a.capturedAt || 0));
                      } else {
                        records.sort((a, b) => (a.capturedAt || 0) - (b.capturedAt || 0));
                      }

                      let idx = 0;
                      function triggerNext() {
                        if (idx < records.length) {
                          const currentVal = records[idx];
                          idx++;
                          const cursor = {
                            value: currentVal,
                            continue: () => {
